...
def create_data_loaders(train_df, val_df, test_df, config):
    ...
    train_dataset = MetaDataset(train_df, data_subdir, config, is_train=True)
    val_dataset = MetaDataset(val_df, data_subdir, config, is_train=False)
    test_dataset = MetaDataset(test_df, data_subdir, config, is_train=False) if test_df is not None else None
