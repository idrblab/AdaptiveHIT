...
# Precomputed ESM-2/X-Mol embedding directories (used by the offline
# training/bulk-evaluation pipeline in meta_run_training.py/predict.py, not
# by single-query inference, which computes embeddings on the fly). Override
# via environment variable or by setting config.protein_emb_dir/drug_emb_dir
# directly; these defaults assume the AdaptiveHIT repo layout.
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_PROTEIN_EMB_DIR = os.environ.get(
    "ADAPTIVEHIT_PROTEIN_EMB_DIR",
    os.path.join(_REPO_ROOT, "data", "embeddings", "esm2_t36_3B_UR50D"),
)
DEFAULT_DRUG_EMB_DIR = os.environ.get(
    "ADAPTIVEHIT_DRUG_EMB_DIR",
    os.path.join(_REPO_ROOT, "data", "embeddings", "xmol"),
)
...
class MetaConfig:
    ...
    # Path configuration
    protein_emb_dir: str = DEFAULT_PROTEIN_EMB_DIR
    drug_emb_dir: str = DEFAULT_DRUG_EMB_DIR
    data_dir: str = "data/"
    data_subdir: str = "alldataset"
    ...
    def print_config_summary(self):
        ...
        print("\n" + "="*80)
        print(f"Meta-learner Configuration Summary - Strategy: {self.strategy}")
        print("="*80)
