...
import numpy as np
from meta_fusion_modules import ProbabilityGuidedAttention, GatedFusion
from meta_encoder_modules import ESM2Encoder, XMolEncoder
...
class MetaLearner(nn.Module):
    ...
    def __init__(self, config):
        ...
        print(f"\n{'='*60}")
        print(f"[MetaLearner] Initializing - Strategy: {config.strategy}")
        print(f"{'='*60}")
        ...
        self.weight_predictor = self._build_meta_network(fusion_output_dim)
        self._init_weights()

        total_params = sum(p.numel() for p in self.parameters())
        print(f"\n[MetaLearner] Total parameters: {total_params:,}")
        print(f"{'='*60}\n")

    def _init_probability_only(self):
        print(f"[MetaLearner] Strategy: probability only")
        print(f"  Input dim: {self.num_models}")

    def _init_full_representations(self):
        print(f"[MetaLearner] Strategy: full representations (ESM-2 + X-Mol)")
    ...
    def _build_meta_network(self, input_dim):
        layers = []
        prev_dim = input_dim
