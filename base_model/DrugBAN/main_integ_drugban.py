...
def main():
    ...
    cfg.RESULT.OUTPUT_DIR = output_model
    ...
    if not cfg.DA.USE:
        trainer = Trainer(model, output_model, opt, device, training_generator, train_dataset, val_generator, test_generator, opt_da=None,
                          discriminator=None,
                          experiment=experiment, **cfg)
    else:
        trainer = Trainer(model, output_model, opt, device, multi_generator, val_generator, test_generator, opt_da=opt_da,
                          discriminator=domain_dmm,
                          experiment=experiment, **cfg)
    eva_dict, result = trainer.train()
    ...
    return eva_dict, result


if __name__ == '__main__':
    s = time()
    result, evalu_dict = main()
    lr_list = [args.lr]*epochs
    evalu_dict['learning_rate'].extend(lr_list)
    evalu_df = pd.DataFrame(evalu_dict)
    evalu_df.to_csv(output, index=False, mode='a')
    e = time()
