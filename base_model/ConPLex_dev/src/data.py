...
def drug_target_collate_fn_test(
    args: T.Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]
):
    """
    Collate function for PyTorch data loader -- turn a batch of triplets into a triplet of batches
    ...
    labels = torch.stack(labs, 0)
    smiles = torch.stack(sms, 0)
    sequences = torch.stack(seqs, 0)

    return drugs, targets, labels, smiles, sequences
...
class BinaryDataset_test(Dataset):
    def __init__(
        self,
        index_smiles,
        index_sequences,
    ...
    def __getitem__(self, i: int):
        ...
        target = self.target_featurizer(self.targets.iloc[i])
        label = torch.tensor(self.labels.iloc[i])
        smiles = torch.tensor(self.index_smiles.iloc[i])
        sequences = torch.tensor(self.index_sequences.iloc[i])

        return drug, target, label, smiles, sequences
...
class DTIDataModule(pl.LightningDataModule):
    ...
    def setup_test(self, stage: T.Optional[str] = None):

        self.df_test = pd.read_csv(
            self._data_dir / self._test_path, **self._csv_kwargs
        )

        unique_smiles = sorted(self.df_test[self._drug_column].unique())
        index_to_smiles_test = {index: smile for index, smile in enumerate(unique_smiles)}
        smiles_to_index_test = {smiles: index for index, smiles in index_to_smiles_test.items()}
        self.df_test["index_smiles"] = self.df_test[self._drug_column].map(smiles_to_index_test)

        unique_sequences = sorted(self.df_test[self._target_column].unique())
        index_to_sequences_test = {index: sequence for index, sequence in enumerate(unique_sequences)}
        sequences_to_index_test = {sequences: index for index, sequences in index_to_sequences_test.items()}
        self.df_test["index_sequences"] = self.df_test[self._target_column].map(sequences_to_index_test)

        self._dataframes = [self.df_test]
        ...
        self.data_test = BinaryDataset_test(
            self.df_test["index_smiles"],
            self.df_test["index_sequences"],
            self.df_test[self._drug_column],
            self.df_test[self._target_column],
            self.df_test[self._label_column],
            self.drug_featurizer,
        ...
        return index_to_smiles_test, index_to_sequences_test, self.df_test

    def test_dataloader_test(self):
        return DataLoader(self.data_test, batch_size=self._loader_kwargs["batch_size"],
                        shuffle= False ,
                        num_workers=self._loader_kwargs["num_workers"],
                        collate_fn=drug_target_collate_fn_test)
