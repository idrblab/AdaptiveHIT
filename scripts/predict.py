...
from adaptivehit_learner import AdaptiveHITModel
from adaptivehit_data_loader import AdaptiveHITDataset, collate_fn
from torch.utils.data import DataLoader


def _repo_root():
    """Repository root: $ADAP_MODEL_ROOT if the shell drivers exported it,
    otherwise the parent of this scripts/ directory."""
...
def override_config_paths(config, input_dir):
    """Point the checkpoint's embedding paths at this checkout.

    A saved checkpoint carries the absolute paths of the machine it was
    trained on, which do not exist anywhere else, so they are always
    replaced here rather than trusted.
    """
    root = _repo_root()
    config.protein_emb_dir = os.path.join(
        root, "_ForFeatures", "esm2", "data", "esm2_t36_3B_UR50D")
    config.drug_emb_dir = os.path.join(
        root, "_ForFeatures", "xmol", "FT_to_embedding", "data", "for_output")
...
class EnsemblePredictor:
    ...
    def _configure_by_strategy(self, config: AdaptiveHITConfig, strategy: str):
        """Configure based on strategy name"""
        if strategy.startswith('adaptivehit_prob_only_'):
            config.use_protein_representation = False
            config.use_drug_representation = False
            config.training_mode = 'normal'
            
        elif strategy.startswith('adaptivehit_full_'):
    ...
    def load_adaptivehit_model(self, strategy: str, model_path: str, input_dir: str = None) -> bool:
        """Load AdaptiveHIT model from checkpoint"""
        try:
            ...
            if 'config' in checkpoint:
                if isinstance(checkpoint['config'], dict):
                    ...
                    config_dict['strategy'] = 'adaptivehit_prob_only' if 'prob_only' in strategy else 'adaptivehit_full'
                    config_dict['data_subdir'] = self.data_subdir
                    config = AdaptiveHITConfig(**config_dict)
                else:
                ...
                if os.path.exists(config_path):
                    ...
                    config = AdaptiveHITConfig(model_names=self.model_names, strategy='adaptivehit_prob_only')
                    config.data_subdir = self.data_subdir

            config = override_config_paths(config, input_dir)
            # The checkpoint records the device it was trained on; honour what
            # this machine actually has instead (a CUDA value would otherwise
            # raise on a CPU-only torch build, which environment.yml installs).
            ...
            print(f"\n    [Model] Creating AdaptiveHITModel...")
            model = AdaptiveHITModel(config)
            ...
            self.adaptivehit_models[strategy] = {
                'model': model,
                'config': config,
                'path': model_path,
    ...
    def create_dataloader(self, data: pd.DataFrame, config: AdaptiveHITConfig, batch_size: int = 512) -> DataLoader:
        """Create data loader for predictions"""
        config.data_subdir = self.data_subdir
        dataset = AdaptiveHITDataset(data, self.data_subdir, config, is_train=False)
    ...
    @torch.no_grad()
    def predict_adaptivehit(self, data: pd.DataFrame, strategy: str, batch_size: int = 512) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray]]:
        """Make predictions using AdaptiveHIT and return weights"""
        if strategy not in self.adaptivehit_models:
            raise ValueError(f"Model {strategy} not loaded")
        
        model_info = self.adaptivehit_models[strategy]
    ...
    def predict_all(self, data: pd.DataFrame, strategies: List[str], batch_size: int = 512) -> pd.DataFrame:
        ...
        for strategy in all_strategies:
            ...
            try:
                ...
                elif strategy in self.adaptivehit_models:
                    proba, labels, weights = self.predict_adaptivehit(data, strategy, batch_size)
                    results[f'Predicted_scores_{strategy}'] = proba
                    results[f'label_predict_{strategy}'] = labels
...
def main():
    ...
    parser.add_argument('--weights_dir', type=str, required=True, help='Directory containing weight files')
    ...
    parser.add_argument('--strategies', type=str, nargs='+', required=True, help='Strategies to evaluate')
    ...
    for s in args.strategies:
        if s == 'average':
            ...
            if model_path:
                predictor.load_adaptivehit_model(s, model_path, args.input_dir)
            else:
