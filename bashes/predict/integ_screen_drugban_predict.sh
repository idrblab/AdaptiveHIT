...
testset_csv_files=$(find "$testset_dir/data" -mindepth 1 -maxdepth 1 -type f -name "*.csv")

for data_sub in $testset_csv_files; do
    data_sub=("$(basename "$data_sub")")
    model_tpye=("$(basename "$model_dir")")
    data_sub_name=("$(basename "$data_sub" .csv)")
    ...
    nohup python predict_drugban.py \
            $batch_size \
            $model_dir \
            predict \
            $data_sub_name \
