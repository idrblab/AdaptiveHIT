...
for mission in "${missions[@]}"; do
    ...
    cd $model_env_dir/dataset/$mission
    mkdir -p models_meta results_meta log_meta weights
    cd $model_env_dir/meta_learner
    batch_size=32
    learning_rate=0.001
    epochs=2
    python meta_run_training.py \
        --dataset $mission \
        --data_subdir $mission \
        --input_dir $data_dir/data/end_merged\
        --output_dir $data_dir/models_meta/meta_full_esm2_xmol_prob_attention-$batch_size-$learning_rate \
